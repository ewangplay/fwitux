						 ...
						 GtkWidget *checkbutton)
{
	...
	fwitux_conf_set_bool (fwitux_conf_get (), conf_path, !hide_hint);
...
gboolean
fwitux_hint_dialog_show (const gchar         *conf_path,
						 ...
						 gpointer             user_data)
{
	...
	ok = fwitux_conf_get_bool (fwitux_conf_get (),
							   conf_path,
							   &show_hint);
...
gboolean 
fwitux_hint_show (const gchar         *conf_path,
				  const gchar         *message1,
				  const gchar         *message2,
				  GtkWindow           *parent,
				  GFunc                func,
				  gpointer             user_data)
{
	return fwitux_hint_dialog_show (conf_path,
									message1, message2,
									parent,
									func, user_data);
}
