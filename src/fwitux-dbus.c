...
dbus_nm_state_cb (DBusGProxy *proxy,
				  ...
				  gpointer    user_data)
{
	fwitux_debug (DEBUG_DOMAIN, "New network state:'%s'",
...
fwitux_dbus_nm_init (void)
{
	...
	fwitux_debug (DEBUG_DOMAIN, "Initialising Network Manager proxy");
...
gboolean
fwitux_dbus_nm_get_state (gboolean *connected)
{
	GError  *error = NULL;
	guint32  state;
	...
	*connected = FALSE;
	...
	if (!dbus_g_proxy_call (nm_proxy, "state", &error,
		...
		fwitux_debug (DEBUG_DOMAIN, "Failed to complete 'state' request. %s",
					  error->message);
		return FALSE;
	}

	fwitux_debug (DEBUG_DOMAIN, "Current network state:'%s'",
				  dbus_nm_state_to_string (state));
