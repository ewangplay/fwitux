...
static gboolean     fwitux_conf_notify_inited = FALSE;
...
static void
spell_notify_languages_cb (FwituxConf  *conf,
						   const gchar *key,
						   gpointer     user_data)
{
	GList *l;

	fwitux_debug (DEBUG_DOMAIN, "Resetting languages due to config change");
...
spell_setup_languages (void)
{
	...
	if (!fwitux_conf_notify_inited) {
		fwitux_conf_notify_add (fwitux_conf_get (),
								FWITUX_PREFS_UI_SPELL_LANGUAGES,
								spell_notify_languages_cb, NULL);

		fwitux_conf_notify_inited = TRUE;
	}

	if (languages) {
		fwitux_debug (DEBUG_DOMAIN, "No languages to setup");
		return;
	}
	...
	if (fwitux_conf_get_string (fwitux_conf_get (),
		...
		while (strv && strv[i]) {
			...
			fwitux_debug (DEBUG_DOMAIN, "Setting up language:'%s'", strv[i]);
...
const char *
fwitux_spell_get_language_name (const char *code)
{
	const gchar *name;
...
GList *
fwitux_spell_get_language_codes (void)
{
...
void
fwitux_spell_free_language_codes (GList *codes)
{
...
gboolean
fwitux_spell_check (const gchar *word)
{
	...
	if (!languages) {
		fwitux_debug (DEBUG_DOMAIN, "No languages to check against");
		return TRUE;
	}
	...
	if (digit) {
		...
		fwitux_debug (DEBUG_DOMAIN, "Not spell checking word:'%s', it is all digits", word);
		return TRUE;
	}
...
GList *
fwitux_spell_get_suggestions (const gchar *word)
{
	gint   len;
	GList *l1;
...
gboolean
fwitux_spell_supported (void)
{
	if (g_getenv ("FWITUX_SPELL_DISABLED")) {
		fwitux_debug (DEBUG_DOMAIN, "FWITUX_SPELL_DISABLE env variable defined");
		return FALSE;
	}

	fwitux_debug (DEBUG_DOMAIN, "Support enabled");
...
fwitux_spell_supported (void)
{
	fwitux_debug (DEBUG_DOMAIN, "Support disabled");
...
fwitux_spell_get_suggestions (const gchar *word)
{
	fwitux_debug (DEBUG_DOMAIN, "Support disabled, could not get suggestions");
...
fwitux_spell_check (const gchar *word)
{
	fwitux_debug (DEBUG_DOMAIN, "Support disabled, could not check spelling");

	return TRUE;
}

const char *
fwitux_spell_get_language_name (const char *lang)
{
	fwitux_debug (DEBUG_DOMAIN, "Support disabled, could not get language name");
...
fwitux_spell_get_language_codes (void)
{
	fwitux_debug (DEBUG_DOMAIN, "Support disabled, could not get language codes");
...
void
fwitux_spell_free_suggestions (GList *suggestions)
{
